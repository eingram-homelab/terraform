...
resource "vsphere_virtual_machine" "vm" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ansible-playbook ~/code/Terraform/files/ansible/tf_deploy_new_server.yaml --extra-vars "group=${var.ansible_group} newhost=${var.vm_name}${count.index + 1}.local.lan newip=${element(var.ip_address_list, count.index)}"
    EOT
  }
}
