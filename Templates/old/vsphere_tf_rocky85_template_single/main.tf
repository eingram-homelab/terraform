...
resource "vsphere_virtual_machine" "vm" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ansible-playbook ~/code/Terraform/files/ansible/tf_deploy_new_server.yaml --extra-vars "group=${var.ansible_group} newhost=${var.vm_name}.local.lan newip=${var.ip_address}"
    EOT
  }
}
...
resource "null_resource" "remove_host" {
  ...
  provisioner "local-exec" {
    when    = destroy
    command = <<-EOT
      ansible-playbook ~/code/Terraform/files/ansible/tf_remove_server_ansible_pihole.yaml --extra-vars "group=${self.triggers.ansible_group} newhost=${self.triggers.vm_name}.local.lan newip=${self.triggers.ip_address}"
    EOT
  }
}